...
pub async fn handle_folders_backup_command(
    ...
    let filters = get_dialog_filters(&tg_client).await?;
...
fn get_filter_id(filter: &tl_types::enums::DialogFilter) -> Option<i32> {
    match filter {
...
fn set_filter_id(filter: &mut tl_types::enums::DialogFilter, id: i32) {
    match filter {
...
pub async fn handle_folders_clear_command(session_file: &path::Path) -> Result<()> {
    ...
    for filter in &filters.filters {
        let maybe_filter_id = get_filter_id(filter);
...
pub async fn handle_folders_restore_command(
    ...
    for saved_filter in &saved_filters.filters {
